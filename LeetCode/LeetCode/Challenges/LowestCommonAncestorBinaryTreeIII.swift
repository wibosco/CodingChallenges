...
struct LowestCommonAncestorBinaryTreeIII {
    ...
    //Treating this tree as a linked list we can race the two pointers to find the root by moving through child ->
    //parent levels. As `p` and `q` can begin at different levels in the tree, simply waiting for `p` and `q` to
    //meet will not guarantee that the lowest common ancestor is where they meet e.g. if `p` starts at level 8 and
    //`q` starts at level 4 then this approach will result in the ancestor being declared as the root when it may
    //be level 3. To overcome this limitation, we need a way to overcome any difference in levels - we can do this
    //with two pointers. First we start both pointers at the levels for `p` and `q` we then navigate up the tree
    //until one of the pointers hits the root at which we reset it to the starting position of the slower
    //pointer, we continue to process the ndoes until the other pointer also hits root at which point it is reset
    //to the quicker pointers starting position. Both pointers will now be at the same level as the difference will
    ...
    //In order for a node to be a common ancestor it needs 2 of the 3 below to be true:
    //
