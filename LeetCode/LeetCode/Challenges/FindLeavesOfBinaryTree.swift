...
struct FindLeavesOfBinaryTree {
    ...
    //the max height of the child nodes at each level to determine what height the current node is at. Using this `height`
    //value we then place the current node into `leaves` at the correct location. By using the height that a node appears
    //at we ensure that all leaf nodes are treated as equal regardless of how deep they are or the order in which we got to
    //them. Once that node has been placed in `leaves` we add 1 to the height and continue back up the recursive call stack.
    //
