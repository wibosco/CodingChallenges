...
struct CountUnivalueSubtrees {
    ...
    @discardableResult
    private func isUnivalueTree(_ root: TreeNode?, _ count: inout Int) -> Bool {
    ...
    //Space: O(h) where h is the height of the tree
    //DFS
    func countUnivalSubtreesPushParentValueDown(_ root: TreeNode?) -> Int {
        guard let root = root else {
            return 0
        }
    ...
    @discardableResult
    private func isUnivalueTreePushParentValueDown(_ root: TreeNode?, _ parentVal: Int, _ count: inout Int) -> Bool {
        guard let root = root else {
    ...
    //Space: O(h) where h is the height of the tree
    //DFS
    func countUnivalSubtreesPushChildValueUp(_ root: TreeNode?) -> Int {
        var count = 0
    ...
    @discardableResult
    private func countUnivalSubtreesPushChildValueUp(_ root: TreeNode?, _ count: inout Int) -> Int? {
        guard let root = root else {
            return nil
        }
