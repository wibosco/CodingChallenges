...
struct LongestSubstringWithoutRepeatingCharacters {
    ...
    //Using a dictionary `map` to store the charcters we have already seen, we iterate through `s`. As we iterate through we keep
    //track of the start of our non-repeating subarray - `currentStartIndex`. For each character we enounter we check if we have
