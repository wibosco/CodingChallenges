...
struct IntersectionOfTwoArraysII {
    ...
    //Time: O(n log n + m log m + min(n, m)) `n` is the number of values in `nums1`, `m` is the number of values in `nums2`
    ...
    //Time: O(n + m) `n` is the number of values in `nums1`, `m` is the number of values in `nums2`
    //Space: O(n + m)
    //dictionary
    //
    //Solution Description
