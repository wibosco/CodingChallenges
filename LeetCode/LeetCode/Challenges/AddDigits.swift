...
struct AddDigits {
    ...
    static func addDigitsRecursive(_ num: Int) -> Int {
        var num = num
        
        while num > 9 {
            num = addIntermidateDigits(num)
        }
        
        return num
    }
    
    private static func addIntermidateDigits(_ num: Int) -> Int {
        guard num > 9 else {
            return num
        }
        ...
        return digit + addIntermidateDigits(num)
    }
}
