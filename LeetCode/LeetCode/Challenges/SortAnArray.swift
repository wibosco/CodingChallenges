...
struct SortAnArray {
    ...
    //Time: O(n * k) `n` is the number of elements in `nums`, k is the number of digits of the largest number
    ...
    //Time: O(n + k) `n` is the number of elements in `nums`, k is the unique elements in `nums`
    //Space: O(k)
    //sorting
    //counting sort
    //dictionary
