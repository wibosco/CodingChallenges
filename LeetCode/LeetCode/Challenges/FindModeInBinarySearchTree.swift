...
struct FindModeInBinarySearchTree {
    ...
    //move onto the next unique value in `nodes`. After all elements in `nodes` have been check we return `modes`.
     static func findMode(_ root: BinaryTreeNode?) -> [Int] {
    ...
    private static func inorder(_ node: BinaryTreeNode?) -> [Int] {
    ...
    //then returned.
    static func findModeFrequency(_ root: BinaryTreeNode?) -> [Int] {
    ...
    private static func dfs(_ node: BinaryTreeNode?, _ frequency: inout [Int: Int], _ maxCount: inout Int) {
        guard let node = node else {
            return
        }
