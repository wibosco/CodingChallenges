...
struct BestTimeToBuyAndSellStock {
    ...
    //through `prices` we subtract the current price from the lowest buying price and compare it against the highest possible
    ...
    //To generate the maximum profit we need to find the largest difference two price elements - as we can't buy in the future
    //and sell in the past that difference needs to be with the low price coming before the high price. To do this we keep track
    //of the highest possible profit that we've come across so far and the highest selling price that we've seen. As we iterate
    //through `prices` we subtract the current price from the highest buying price and compare it against the highest possible
    //profit value that we have seen so far - if it larger then we update with the current difference and if lower we discard.
    //We are always looking for a higher price so when we encounter one we update our highest buying price value.
    //
    //See: https://en.wikipedia.org/wiki/Maximum_subarray_problem#Kadane's_algorithm
    func maxProfit2(_ prices: [Int]) -> Int {
        ...
        var p1 = prices.count - 2

        var maxPrice = prices[prices.count - 1]
        var maxProfit = 0

        while p1 >= 0 {
            let profit = maxPrice - prices[p1]
            maxProfit = max(maxProfit, profit)
            maxPrice = max(maxPrice, prices[p1])
