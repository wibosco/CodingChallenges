...
struct MaximumSwap {
    ...
    //In order to get the maximum `num` value we want to the swap a left element with the highest value right-most element i.e
    //swap low value more-significant element with high value less-significant value. To enable to efficiently search the digits
    //in `num` for that higher value element we first convert `num` into an array of individual digits and then store the
    //indexes of individual digits in an array. As we want to swap a higher value element that is as right as possible, if an
    //element is repeated we only store it's last index (right most). Next we iterate through the digits of `num` and attempt
    //to find that replacement. We have to be careful that any replacement we find is larger than the current element and from
    //later in the digits array. If we find that element we swap the current element with it and return as that is guaranteed to
    //be the largest configuration of `num`. If we can't find a replacement for any of the digits in `num` i.e. `num` is in
    //descending or equal order then we simply return `num`
    func maximumSwap(_ num: Int) -> Int {
        var nums = String(num).map { $0.wholeNumberValue! }
        var indexes = Array(repeating: -1, count: 10)
        ...
        for (i, num) in nums.enumerated() {
            for swapee in ((num + 1)..<10).reversed() { // +1 as we only want to swap a larger value
                let si = indexes[swapee]
                if si > i {
                    nums.swapAt(i, si)
                
                    return nums.reduce(0) { $0 * 10 + $1 }
                }
            }
        }
    ...
    //simply return `num`
    func maximumSwap2(_ num: Int) -> Int {
    ...
    func maximumSwap3(_ num: Int) -> Int {
        var maxNum = num
