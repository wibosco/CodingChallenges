...
class ClosestBinarySearchTreeValueTests: XCTestCase {
    ...
    func test_A() {
        let target = 3.714286
        let root = TreeNode.createBinaryTree(fromBFSArray: [4, 2, 5, 1, 3])
    ...
    func test_B() {
        let target = 4.428571
        let root = TreeNode.createBinaryTree(fromBFSArray: [1])
    ...
    func test_C() {
        let target = 3.428571
        let root = TreeNode.createBinaryTree(fromBFSArray: [1, nil, 3])
