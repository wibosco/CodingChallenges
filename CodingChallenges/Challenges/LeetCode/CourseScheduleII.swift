...
struct CourseScheduleII {
    ...
    //Using an adjacency list buld up the out going edges for each course to then use during BFS traversal. Also build up a count of
    //the in-degree for each course - in order for a course to be taken, all prerequisite course must have been taken first, the
    //`inDegrees` array will ensure this. As we can have multiple starting courses (courses with no prerequisities i.e in-degree == 0)
    //we add these first to the queue. Using BFS we then traverse the graph adding new courses to the queue when they no longer
